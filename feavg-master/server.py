...
class Server(object):
    def __init__(self, conf, eval_dataset):
      ...
      self.test_model = models.get_model(self.conf["model_name"])
      # 生成一个测试集合加载器
    ...
    def cient_modeltest(self, diff):
        ...
        for name, param in self.global_model.state_dict().items():
            # 用服务器下发的全局模型覆盖本地模型
            self.test_model.state_dict()[name].copy_(param.clone())

        for name, data in self.test_model.state_dict().items():
            # 更新每一次乘以配置文件中的学习率
            update_per_laye = diff[name] * self.conf["lambda"]
            # 累加
            if data.type() != update_per_laye.type():
                # 因为update_per_layer的type是floatTensor，所以将其转换为模型的LongTensor（损失精度）
                data.add_(update_per_laye.to(torch.int64))
            else:
                data.add_(update_per_laye)
        disloss = models.model_norm(self.test_model, self.global_model)
        cos = models.model_cos(self.test_model, self.global_model)
        # pdb.set_trace()
        ...
        return acc, total_l, disloss, cos
